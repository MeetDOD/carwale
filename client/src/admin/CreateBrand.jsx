...
const CreateCategory = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.post('http://localhost:5000/api/brand/create-brand', { name })
            if (data?.success) {
                alert('Brand Created')
                getAllBrand()
            } else {
    ...
    const handleUpdate = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.put(`http://localhost:5000/api/brand/update-brand/${selected._id}`, { name: updatedName })
            if (data?.success) {
                alert('Brand Updated')
                setSelected(null)
                setUpdatedName("")
    ...
    const handleDelete = async (id) => {
        try {
            const { data } = await axios.delete(`http://localhost:5000/api/brand/delete-brand/${id}`, { name: updatedName })
            if (data?.success) {
                alert(`${name} Deleted`)
                getAllBrand()
            } else {
